...
const isPopup = window.location.search.includes( 'type=action' );
...
const currentLanguage = options.hasOwnProperty( 'preferredLanguage' ) || navigator.language;
...
document.addEventListener( 'change', event => {
	...
	if ( event.target.id == 'importpicker' ) {
		...
		if ( origins.length > 0 || permissions.length > 0 ) {
			...
			const permissionsToRequest = { origins: origins, permissions: permissions };
...
async function ValidatePermission( element ) {
	...
	const permissionsToRequest = { origins: [ permission ] };
...
async function ValidateContentScript( element ) {
	...
	const js = element.getAttribute( 'contentScript' );
	const id = element.getAttribute( 'contentScriptID' );
	if ( js != null && id != null ) {
...
async function OpenImportTab() {
	...
	const permissionsToRequest = { permissions: [ 'tabs' ] };
	const response = await browser.permissions.request( permissionsToRequest );
	if ( response == true ) {
...
function AndroidImportReplacer() {
	...
	browser.permissions.contains( { permissions: [ 'tabs' ] } ).then( ( value ) => {
		...
		const reminder = document.getElementById( 'tabpermissionreminder' );
...
async function RequestAllMissingPermissions() {
	...
	if ( missingHostPermissions.length > 0 ) {
		...
		const permissionsToRequest = { origins: missingHostPermissions };
