...
browser.runtime.onMessage.addListener((msg, sender, response) => {
	...
	if (msg.name == 'GETDATA') { // Standard call
		var options = null;
		if (msg.options !== null) { options = msg.options; }
		...
		try {
			(async () => {
				...
				if (await CheckForPermission(msg.url)) {
					fetch(encodeURI(msg.url), options).then(async res => {
						let errors = null;

						// Check for errors
						if (res.status !== 200) {
							if (res.errors !== null) { errors = res.errors; }
...
async function registerContentScripts() {
	await browser.storage.sync.get('options', async data => {
		if (data !== null) {
			const storedSettings = data.options;
...
async function ConvertLocalToSync() {
	// Get from local
	const options = await browser.storage.local.get().then(storedSettings => storedSettings);
...
browser.runtime.onInstalled.addListener(async details => {
	if (details.reason == 'install') {
		...
		const version = details.previousVersion.split('.');
