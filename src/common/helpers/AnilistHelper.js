class AniListHelper extends Helper {

	static fetchURL = 'https://graphql.anilist.co';
	url = '';
	loadState = Helper.LOAD_STATES['Uninitialized'];
	id = null;
	data = null;
	highest = 0;
	num_ratings = 0;
	...
	_addAnilistRatings() {
		...
		if (this.data.stats.scoreDistribution.length == 10) {
			for (var ii = 0; ii < 10; ii++) {
				...
				if (amount > this.highest) { this.highest = amount; }
	...
	fetchData() {
		...
		try {
			this.loadState = Helper.LOAD_STATES['Loading'];
			browser.runtime.sendMessage({ name: 'GETDATA', type: 'JSON', url: fetchURL, options: options }, value => {
			...
			if (al && al.data != null) {
					...
					if (this.data != null) {
						...
						this.loadState = Helper.LOAD_STATES['Success'];
						this.loadSuccessCallback(this.url, 'AL', 'al-button');
						this._addAnilistRatings();
					} else {
						this.loadState = Helper.LOAD_STATES['Failure']
					}
				} else {
					this.loadState = Helper.LOAD_STATES['Failure']
