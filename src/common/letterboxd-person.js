class LetterboxdPerson {
	...
	updateLostFilms() {
		...
		for (let i = 0; i < films.length; i++) {
			...
			if (filmWatched == null) { filmWatched = ''; }
			...
			if (this.lostFilms.list.includes(filmID) && hide == 'hide') {
				...
				if (filmWatched.toLowerCase() == 'true') { this.lostFilms.watchedCount++; }

				if (film.className.includes('extras-lost-film')) { film.className = film.className.replace(' extras-lost-film', ''); }
			}
		...
		if (document.querySelector('.sidebar .actions .progress-panel') !== null) {
			...
			if (originalTotal != this.lostFilms.visibleCount) { progressCounter.append(` / ${originalTotal} total`); }
		...
		if (extrasuiHeader !== null) { extrasuiHeading = extrasuiHeader.querySelector('.ui-block-heading'); }
		...
		if (extrasuiHeader == null) {
			...
			var removeLink = this.extensionHelpers.createElement('a', {
		...
		if (this.lostFilms.enabled && this.lostFilms.lostFilmCount > 0) {
			if (uiHeader !== null) { uiHeader.style.display = 'none'; } 
			extrasuiHeader.style.display = '';
		} else {
			if (uiHeader !== null) { uiHeader.style.display = ''; }
