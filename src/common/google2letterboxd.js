...
async function google2letterboxd() {
	// Get storage
	const options = await browser.storage.sync.get().then(storedSettings => {
	...
	imdbElements.forEach(imdbElement => {
		...
		if (!letterboxdElement) {
			...
			if (headingElement) { headingElement.parentElement.style.display = 'none'; }

			// Remove links if there are too many
			if (imdbLink.parentElement.childElementCount >= 5) {
				for (let i = imdbLink.parentElement.childElementCount - 1; i >= 3; i--) { imdbLink.parentElement.children[i].remove(); }
			}
			...
			fetch(letterboxdUrl)
				...
				.then(text => {
					...
					if (ratingMatch) {
						...
						if (headingElement) { headingElement.parentElement.style.display = 'block'; }
					} else {
			...
			const rating = parseFloat(span1.textContent.split('/')[0]);
			span1.textContent = `${(rating * 2).toFixed(1)}/10`;
		}
	});
}
